...
export async function saveBooking({ tier, quantity, paymentMethod, totalAmount, eventId }) {
  let bookingId;
  let seq = 1;
...
export async function fetchBookingByTicketNumber(ticketNumber) {
  try {
    const q = query(collection(firestore, FIRESTORE_BOOKINGS), where("ticketNumber", "==", ticketNumber));
    const snap = await getDocs(q);
